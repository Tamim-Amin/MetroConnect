...
?>
    <div class="lg:grid lg:grid-cols-3 gap-6">
        <aside class="hidden lg:block">
            <div class="bg-white p-4 rounded-xl card-shadow fade-up">
        ...
        <section class="col-span-2 space-y-6">
            <div class="bg-white p-5 rounded-xl card-shadow fade-up">
                <form id="postForm" method="POST" action="/post" enctype="multipart/form-data" class="space-y-4">
                    <textarea name="content" id="content" rows="3" placeholder="What's happening?" class="w-full resize-none border rounded p-3"></textarea>
                    ...
                    <div class="flex items-center gap-3">
                        <label class="flex items-center gap-2 cursor-pointer text-sm text-slate-600">
                            ...
                            <span class="px-3 py-2 bg-slate-100 rounded inline-flex items-center gap-2">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7l6 6 4-4 8 8" /></svg>
              Attach image
            </span>
                        </label>
            ...
            <div id="feed" class="space-y-4">
                ...
                <?php foreach ($posts as $post): ?>
                    <article class="bg-white p-5 rounded-xl card-shadow fade-up">
                        <div class="flex items-start gap-4">
                            ...
                            <div class="flex-1">
                                ...
                                <div class="mt-3 flex items-center gap-4 text-sm text-slate-500">
                                    ...
                                    <?php
                                    // show edit link only to post owner and within 24 hours of creation
                                    $canEdit = true;
                                    if (isset($user) && isset($post['user_id']) && (int)$user['id'] === (int)$post['user_id']) {
                                        $created = strtotime($post['created_at']);
                                        if (time() - $created <= 24 * 60 * 60) $canEdit = true;
                                    }
                                    if ($canEdit): ?>
